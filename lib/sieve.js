...
startedPromise.then(function(){
	...
	var ensureFreshCache = function(){
		logger.info("Ensuring cache freshness.");
		releaseService.getRelease('current')
			.then(function(release){
				particleService.getParticles(release);
			})
			.fail(function(err){
				logger.error(err, "failed to freshen cache");
			});
	};

	setInterval(ensureFreshCache, config.cache.ttl * 1.1);
	ensureFreshCache();
});